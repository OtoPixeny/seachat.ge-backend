...
// Function to emit online users to all clients
function emitOnlineUsers() {
  const onlineIds = Array.from(connectedUsers.values())
    .filter(user => user.userId)
    .map(user => user.userId);
  io.emit('onlineUsers', onlineIds);
  logEvent('ONLINE_USERS_UPDATE', 'system', { 
...
io.on('connection', (socket) => {
  ...
  socket.on('authenticate', async (token, callback) => {
    try {
      logEvent('AUTH_ATTEMPT', socket.id, { hasToken: !!token });
      
      // Verify the token with Supabase
      const { data: { user }, error } = await supabase.auth.getUser(token);
      
      if (error || !user) {
        logEvent('AUTH_ERROR', socket.id, { error: error?.message || 'Invalid token' });
      ...
      logEvent('AUTH_SUCCESS', socket.id, { userId: user.id });
      socket.emit('authenticated', { userId: user.id });
      if (callback) callback({ success: true, userId: user.id });
      emitOnlineUsers();
      
    } catch (error) {
      logEvent('AUTH_ERROR', socket.id, { error: error.message });
      socket.emit('authError', { error: 'Authentication failed' });
      if (callback) callback({ error: 'Authentication failed' });
    }
  });
  ...
  socket.on('sendMessage', async (messageData, callback) => {
    try {
      logEvent('SEND_MESSAGE_ATTEMPT', socket.id, { 
        messageData: { ...messageData, content: messageData.content?.substring(0, 50) + '...' },
        isAuthenticated: connectedUsers.get(socket.id)?.authenticated
      });
      
      // Check if user is authenticated
      const userData = connectedUsers.get(socket.id);
      if (!userData || !userData.authenticated || !userData.token) {
        logEvent('ERROR', socket.id, { error: 'User not authenticated', userData });
        const errorMsg = 'Authentication required to send messages';
      ...
      if (!sender_id || !receiver_id || !content) {
        ...
        const errorMsg = 'Missing required fields: sender_id, receiver_id, or content';
        socket.emit('messageError', { error: errorMsg });
        if (callback) callback({ error: errorMsg });
        return;
      }

      // Verify that the authenticated user matches the sender_id
      if (userData.userId !== sender_id) {
        logEvent('ERROR', socket.id, { 
          error: 'Sender ID mismatch', 
          authenticatedUser: userData.userId,
          senderId: sender_id
        });
        const errorMsg = 'You can only send messages as yourself';
        socket.emit('messageError', { error: errorMsg });
        if (callback) callback({ error: errorMsg });
        return;
      }
      ...
      if (content.length > 1000) {
        ...
        const errorMsg = 'Message exceeds maximum length of 1000 characters';
        socket.emit('messageError', { error: errorMsg });
        if (callback) callback({ error: errorMsg });
        return;
      }
      ...
      const authenticatedSupabase = createAuthenticatedClient(userData.token);

      // Insert message into Supabase with authentication
